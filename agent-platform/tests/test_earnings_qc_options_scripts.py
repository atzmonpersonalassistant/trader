...
class EarningsQcOptionsGeneratedCodeTests(unittest.TestCase):
    ...
    def test_stage2_relaxes_oi_volume_and_absolute_spread_gates_with_diagnostics(self):
        ...
        self.assertEqual(main.count('"expiry_within_0_7d_after_earnings": 0'), 1)
        self.assertIn('"otm_strike_available": 0', main)
        self.assertIn('self.funnel["otm_strike_available"] += 1', main)
        self.assertIn("strike_spot_ratio", main)
        self.assertIn("spot_unavailable", main)
        self.assertIn("no_two_sided_market", main)
