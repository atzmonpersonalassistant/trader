...
class MVP0AgentTests(unittest.TestCase):
    def test_bootstrap_new_vps_script_static_validation(self):
        ...
        self.assertIn('LEAN_CLI_VERSION=1.0.225', text)
        self.assertIn('version("lean")', text)
        self.assertIn('[[ "$installed_lean_version" != "$LEAN_CLI_VERSION" ]]', text)
        self.assertIn('python3 -m pip install --break-system-packages "lean==$LEAN_CLI_VERSION"', text)
        self.assertNotIn('python3 -m pip install --break-system-packages --upgrade lean', text)
        ...
        self.assertIn('chmod 750 /agents/research /agents/research/state /agents/research/logs /agents/research/reports /agents/research/tmp /agents/research/lean-workspace', text)
    ...
    def test_research_agent_blocks_event_candidate_without_ready_provider(self):
        research = load("trading_research_agent_event_gate", "agent-platform/tools/trading_research_agent.py")
        old_require = os.environ.get("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER")
        old_ready = os.environ.get("TRADING_RESEARCH_EVENT_PROVIDER_READY_FILE")
        with TemporaryDirectory() as tmp:
            ready_file = Path(tmp) / "missing-ready"
            queue = Path(tmp) / "strategy-queue.json"
            queue.write_text(json.dumps([{
                "id": "event-candidate-without-date",
                "priority": 1,
                "status": "queued",
                "family": "calendar_spread",
            ...
            try:
                ...
                out = io.StringIO()
                with contextlib.redirect_stdout(out):
                    rc = research.cmd_claim(argparse.Namespace(queue=str(queue), run_id="run-event"))
            finally:
                if old_require is None:
                    os.environ.pop("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER", None)
                else:
    ...
    def test_research_agent_allows_event_candidate_with_explicit_event_date(self):
        ...
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                out = io.StringIO()
                with contextlib.redirect_stdout(out):
                    rc = research.cmd_claim(argparse.Namespace(queue=str(queue), run_id="run-event-date"))
            finally:
                if old_require is None:
                    os.environ.pop("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER", None)
                else:
    ...
    def test_research_agent_blocks_event_candidate_with_only_target_expiry_date(self):
        research = load("trading_research_agent_expiry_is_not_event_date", "agent-platform/tools/trading_research_agent.py")
        old_require = os.environ.get("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER")
        old_ready = os.environ.get("TRADING_RESEARCH_EVENT_PROVIDER_READY_FILE")
        with TemporaryDirectory() as tmp:
            ready_file = Path(tmp) / "missing-ready"
            queue = Path(tmp) / "strategy-queue.json"
            queue.write_text(json.dumps([{
                "id": "event-candidate-with-expiry-only",
                "priority": 1,
                "status": "queued",
                "family": "calendar_spread",
                "thesis": "earnings catalyst window requires calendar data",
            ...
            try:
                ...
                out = io.StringIO()
                with contextlib.redirect_stdout(out):
                    rc = research.cmd_claim(argparse.Namespace(queue=str(queue), run_id="run-event-expiry"))
            finally:
                if old_require is None:
                    os.environ.pop("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER", None)
                else:
    ...
    def test_research_agent_blocks_event_candidate_with_only_exit_expiration_date(self):
        research = load("trading_research_agent_exit_expiration_not_event_date", "agent-platform/tools/trading_research_agent.py")
        old_require = os.environ.get("TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER")
        old_ready = os.environ.get("TRADING_RESEARCH_EVENT_PROVIDER_READY_FILE")
        with TemporaryDirectory() as tmp:
            ready_file = Path(tmp) / "missing-ready"
            queue = Path(tmp) / "strategy-queue.json"
            queue.write_text(json.dumps([{
                "id": "event-candidate-with-exit-expiration-only",
                "priority": 1,
                "status": "queued",
                "family": "calendar_spread",
                "thesis": "earnings catalyst window requires calendar data",
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        runner_text = Path("agent-platform/runtime/bin/trading-research-runner-codex").read_text()
        ...
        self.assertIn("--sandbox workspace-write", runner_text)
        self.assertNotIn("sandbox_workspace_write.network_access=true", runner_text)
        self.assertIn('approval_policy="never"', runner_text)
        ...
        self.assertNotIn('install -o root -g root -m 755 "${SCRIPTS_DIR}/trading-research-qc-docker-run" /usr/local/sbin/trading-research-qc-docker-run', bootstrap_text)
        self.assertIn('rm -f /etc/sudoers.d/trading-agent-research-qc-docker /usr/local/sbin/trading-research-qc-docker-run /etc/trading-agents/qc-lean-docker-image', bootstrap_text)
        self.assertNotIn('quantconnect/research:latest', bootstrap_text)
        self.assertNotIn('usermod -aG docker agent-research', bootstrap_text)
        self.assertIn("os.path.realpath", runner_text)
        self.assertIn("idea-generation-*-task.txt", runner_text)
        watchdog_text = Path("agent-platform/runtime/bin/trading-research-watchdog-codex").read_text()
        self.assertIn("research-watchdog-*-task.txt", watchdog_text)
        ...
        self.assertIn("/agents/research/reports/idea-generation-*", runner_text)
        self.assertIn("/agents/research/reports/research-watchdog-*", watchdog_text)
        ...
        self.assertIn("QC_CLOUD_EXTRACT_MAX_ATTEMPTS=${QC_CLOUD_EXTRACT_MAX_ATTEMPTS:-3}", broker_text)
        self.assertIn("QC_CLOUD_EXTRACT_ATTEMPT", broker_text)
        self.assertIn('manifest["attempt_count"] = len(surface_attempts)', broker_text)
        self.assertIn("auth_failure", broker_text)
        self.assertIn("lean_missing", broker_text)
        self.assertIn('docker_status="removed"', broker_text)
        ...
        self.assertNotIn("TRADING_RESEARCH_QC_LEAN_DOCKER_IMAGE", broker_text)
        self.assertNotIn("TRADING_RESEARCH_QC_LEAN_DOCKER_IMAGE_CONFIG", broker_text)
        self.assertNotIn("/etc/trading-agents/qc-lean-docker-image", broker_text)
        self.assertNotIn("TRADING_RESEARCH_QC_LEAN_DOCKER_WRAPPER", broker_text)
        ...
        self.assertNotIn('sudo -n "$QC_LEAN_DOCKER_WRAPPER" "$RUN_REAL" "$docker_image"', broker_text)
        ...
        self.assertIn("qc_cloud_execution_surface_unavailable", broker_text)
        ...
        self.assertIn('"auth_status": "not_checked"', broker_text)
        self.assertIn('"cloud_status": "not_checked"', broker_text)
        self.assertIn('"docker_status": "not_checked"', broker_text)
        self.assertIn('"execution_surface": None', broker_text)
        self.assertIn('"execution_rc": None', broker_text)
        self.assertIn('"surface_attempts": []', broker_text)
        self.assertIn('manifest["auth_status"] = auth_status', broker_text)
        self.assertIn('manifest["cloud_status"] = cloud_status', broker_text)
        self.assertIn('manifest["docker_status"] = docker_status', broker_text)
        self.assertIn('manifest["execution_surface"] = execution_surface', broker_text)
        self.assertIn('manifest["execution_rc"] = maybe_int(execution_rc)', broker_text)
        self.assertIn('manifest["surface_attempts"] = surface_attempts', broker_text)
        self.assertIn('execution_surface="none_attempted"', broker_text)
        self.assertIn('append_surface_attempt "qc_cloud_backtest"', broker_text)
        self.assertNotIn('append_surface_attempt "local_python_quantbook_script"', broker_text)
        self.assertNotIn('append_surface_attempt "lean_docker_quantconnect_python_runtime"', broker_text)
        self.assertNotIn("docker_command_attempted=", broker_text)
    ...
    def test_research_qc_broker_research_artifact_manifest_is_truthful(self):
        ...
        self.assertIn('"attempted_command": "timeout ${QC_CLOUD_EXTRACT_WRAPPER_TIMEOUT_SECONDS}s trading-research-qc-cloud-extract RUN_DIR"', text)
        ...
        self.assertNotIn("TRADING_RESEARCH_QC_LEAN_DOCKER_IMAGE", text)
        self.assertNotIn("trading-research-qc-docker-run", text)
        ...
        self.assertIn('manifest["auth_status"] = auth_status', text)
        self.assertIn('manifest["cloud_status"] = cloud_status', text)
        self.assertIn('manifest["docker_status"] = docker_status', text)
        self.assertIn('manifest["execution_surface"] = execution_surface', text)
        self.assertIn('manifest["execution_rc"] = maybe_int(execution_rc)', text)
        self.assertIn('manifest["surface_attempts"] = surface_attempts', text)
        self.assertIn('manifest["attempt_count"] = len(surface_attempts)', text)
        self.assertIn('execution_surface="none_attempted"', text)
        self.assertIn('append_surface_attempt "qc_cloud_backtest"', text)
        self.assertNotIn('append_surface_attempt "local_python_quantbook_script"', text)
        self.assertNotIn('append_surface_attempt "lean_docker_quantconnect_python_runtime"', text)
    ...
    def test_research_agent_lean_setup_plan_has_no_secret_values(self):
        ...
        self.assertIn("python3 -m pip install --user lean==1.0.225", commands)
        self.assertIn("python3 -m pipx install lean==1.0.225", commands)
        self.assertNotIn("python3 -m pip install --user --upgrade lean", commands)
    ...
    def test_vps_deploy_logs_lean_in_as_agent_research(self):
        ...
        self.assertIn("install -d -m 700 '$DEPLOY_DIR'", text)
        self.assertIn("scp -p -i ~/.ssh/vps_key", text)
        self.assertIn("trap cleanup_deploy_dir EXIT", text)
        self.assertIn("sweep_stale_deploy_dirs() {", text)
        self.assertIn("for dir in /tmp/trader-agent-tools-*; do", text)
        self.assertIn('if [[ "$dir" != "$DEPLOY_DIR" ]]; then', text)
        self.assertIn('rm -rf -- "$dir"', text)
        self.assertIn('stale deploy dir remains after cleanup: $dir', text)
        self.assertIn("sweep_stale_deploy_dirs", text)
        self.assertIn('test "$(stat -c \'%a\' "$DEPLOY_DIR")" = "700"', text)
        self.assertIn('test "$(stat -c \'%a\' "$DEPLOY_DIR/quantconnect.env")" = "600"', text)
        self.assertIn("lean whoami", text)
        self.assertIn("trading-research-qc-broker preflight", text)
        self.assertIn("trading-research-qc-broker research-artifact", text)
        ...
        self.assertIn("trading-research-runner-codex", text)
        self.assertIn("runner user can read QuantConnect secrets", runner_text)
        self.assertIn("umask 0007", runner_text)
        ...
        self.assertIn("systemctl is-enabled --quiet trading-research-agent.service", text)
        self.assertIn("systemctl is-active --quiet trading-research-agent.service", text)
        self.assertIn("systemctl try-restart trading-research-agent.service", text)
        self.assertNotIn("systemctl restart trading-research-agent.service", text)
        self.assertNotIn("systemctl list-unit-files --type=service", text)
        self.assertIn("latest_smoke_dir=", text)
        self.assertLess(text.index("latest_smoke_dir="), text.index("systemctl try-restart trading-research-agent.service"))
        self.assertIn("tee \"$deploy_output\"", text)
        self.assertIn("grep -q '^DEPLOY_OK$' \"$deploy_output\"", text)
    ...
    def test_coding_agent_prompt_allows_code_and_rejects_docs_only_downgrade(self):
        ...
        self.assertFalse(coding.is_allowed_mvp0_change("agent-platform/scripts/trading-research-qc-docker-run"))
    ...
    def test_coding_agent_enforces_safe_agent_platform_changes(self):
        ...
        self.assertFalse(agent.is_allowed_mvp0_change("agent-platform/scripts/trading-research-qc-docker-run"))
    ...
    def test_vps_deploy_installs_quantconnect_secret_env(self):
        ...
        self.assertNotIn('agent-platform/scripts/trading-research-qc-docker-run', workflow)
        ...
        self.assertNotIn('sudo install -o root -g root -m 755 "$DEPLOY_DIR/trading-research-qc-docker-run" /usr/local/sbin/trading-research-qc-docker-run', workflow)
        self.assertIn('sudo rm -f /etc/sudoers.d/trading-agent-research-qc-docker /usr/local/sbin/trading-research-qc-docker-run /etc/trading-agents/qc-lean-docker-image', workflow)
        self.assertNotIn('quantconnect/research:latest', workflow)
        ...
        self.assertNotIn("agent-research ALL=(root) NOPASSWD: /usr/local/sbin/trading-research-qc-docker-run *", workflow)
        self.assertNotIn("bash -n /usr/local/sbin/trading-research-qc-docker-run", workflow)
        self.assertIn('test ! -e /usr/local/sbin/trading-research-qc-docker-run', workflow)
        self.assertIn('test ! -e /etc/sudoers.d/trading-agent-research-qc-docker', workflow)
