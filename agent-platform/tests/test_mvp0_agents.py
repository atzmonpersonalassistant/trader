...
class MVP0AgentTests(unittest.TestCase):
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        self.assertIn("skip static seed: event provider not configured/ready", text)
        self.assertIn("event_provider_allows_static_seed", text)
        self.assertLess(text.index("event_provider_allows_static_seed"), text.index('trading-research-agent --queue "$QUEUE" seed-cheap-calls'))
