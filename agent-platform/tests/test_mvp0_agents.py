...
class MVP0AgentTests(unittest.TestCase):
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        self.assertIn('append_surface_attempt "local_python_quantbook_script"', broker_text)
        self.assertIn('append_surface_attempt "lean_docker_quantconnect_python_runtime"', broker_text)
        self.assertIn("docker_command_attempted=false", broker_text)
        self.assertIn("docker_command_attempted=true", broker_text)
        self.assertLess(
            broker_text.index('execution_status="extract_unavailable"'),
            broker_text.index('timeout 120s python3 qc_option_history_probe.py'),
        )
    ...
    def test_research_qc_broker_research_artifact_manifest_is_truthful(self):
        ...
        self.assertIn('append_surface_attempt "local_python_quantbook_script"', text)
        self.assertIn('append_surface_attempt "lean_docker_quantconnect_python_runtime"', text)
