...
class MVP0AgentTests(unittest.TestCase):
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        self.assertIn("trading-research-qc-smoke --json > \"$RUN_DIR/qc_auth_smoke.jsonl\"", text)
        self.assertIn("qc_auth_smoke.err", text)
        self.assertIn("SMOKE_RC=$?", text)
        self.assertIn("QC_AUTH_OK", text)
        self.assertIn("continue to option-chain and backtest diagnostics", text)
        ...
        self.assertIn("sandbox_workspace_write.network_access=true", bootstrap_text)
    ...
    def test_vps_deploy_logs_lean_in_as_agent_research(self):
        ...
        self.assertIn("lean whoami", text)
        self.assertIn("trading-research-qc-smoke --json >/tmp/trader-research-qc-smoke.jsonl", text)
        self.assertIn('"result":"QC_AUTH_OK"', text)
        self.assertIn("/agents/coding/lean-workspace", text)
        self.assertIn("/agents/review/lean-workspace", text)
        self.assertIn("/agents/validator/lean-workspace", text)
        self.assertIn("/agents/shared/lean-projects", text)
        ...
        self.assertIn("trading-research-runner-codex", text)
        self.assertIn("sandbox_workspace_write.network_access=true", text)
    ...
    def test_vps_deploy_installs_quantconnect_secret_env(self):
        ...
        self.assertIn("sandbox_workspace_write.network_access=true", workflow)
