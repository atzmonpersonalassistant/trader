...
class MVP0AgentTests(unittest.TestCase):
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        self.assertIn('manifest["execution_rc"] = int(execution_rc)', broker_text)
        self.assertIn('manifest["surface_attempts"] = surface_attempts', broker_text)
        self.assertIn('"surface": "qc_cloud_backtest"', broker_text)
        self.assertIn('"surface": "local_python_quantbook_script"', broker_text)
        self.assertIn('"surface": "lean_docker_quantconnect_python_runtime"', broker_text)
        self.assertIn("docker_command_attempted=false", broker_text)
        self.assertIn("docker_command_attempted=true", broker_text)
        self.assertIn('if local_execution_rc != "":', broker_text)
        self.assertIn('if docker_command_attempted == "true":', broker_text)
        self.assertLess(
            broker_text.index('execution_surface="local_python_quantbook_script"'),
    ...
    def test_research_qc_broker_research_artifact_manifest_is_truthful(self):
        ...
        self.assertIn('manifest["execution_rc"] = int(execution_rc)', text)
        self.assertIn('manifest["surface_attempts"] = surface_attempts', text)
        self.assertIn('"surface": "qc_cloud_backtest"', text)
        self.assertIn('if local_execution_rc != "":', text)
        self.assertIn('if docker_command_attempted == "true":', text)
