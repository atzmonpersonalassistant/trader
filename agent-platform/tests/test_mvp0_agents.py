...
class MVP0AgentTests(unittest.TestCase):
    ...
    def test_research_loop_uses_runner_without_raw_qc_secret_access(self):
        ...
        self.assertIn("REQUIRE_EVENT_PROVIDER=${TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER:-1}", text)
        ...
        self.assertIn('skip $action: event provider not configured/ready', text)
        self.assertIn("event_provider_ready_or_optional", text)
        self.assertLess(text.index("event_provider_ready_or_optional"), text.index('trading-research-agent --queue "$QUEUE" seed-cheap-calls'))
        self.assertLess(text.index("REQUIRE_EVENT_PROVIDER=${TRADING_RESEARCH_REQUIRE_EVENT_PROVIDER:-1}"), text.index('trading-research-agent --queue "$QUEUE" generate-ideas'))
        self.assertIn('if event_provider_ready_or_optional "static seed"; then', text)
        self.assertIn('if [ "$DISABLE_SEED" != "1" ] && event_provider_ready_or_optional "idea generation"; then', text)
