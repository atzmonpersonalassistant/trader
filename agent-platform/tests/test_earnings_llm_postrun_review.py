...
class EarningsLlmPostrunReviewTests(unittest.TestCase):
    ...
    def test_daily_selector_requires_explicit_daily_stage_parameters(self):
        ...
        self.assertIn("COALESCE((parameters_json->>'no_outbox')::boolean, false) = false", text)
        self.assertEqual(text.count("COALESCE((parameters_json->>'no_outbox')::boolean, false) = false"), 2)
