...
class VpsDeployWorkflowTests(unittest.TestCase):
    ...
    def test_earnings_research_has_single_public_cli(self):
        ...
        self.assertIn('agent-platform/scripts/earnings-qc-options/earnings-qc-daily-run.sh', workflow)
        self.assertIn('/usr/local/bin/earnings-qc-daily-run.sh', workflow)
        self.assertIn("grep -q '/agents/research/bin/earnings-qc-research' /usr/local/bin/earnings-qc-daily-run.sh", workflow)
        self.assertIn("! grep -q '/agents/research/bin/earnings-qc-options-full-scan' /usr/local/bin/earnings-qc-daily-run.sh", workflow)
        self.assertIn('sudo rm -f /usr/local/bin/earnings-otm-daily-root.sh', workflow)
        self.assertIn('test ! -e /usr/local/bin/earnings-otm-daily-root.sh', workflow)
        self.assertIn('sudo crontab -l', workflow)
        self.assertIn('earnings-otm-daily-root', workflow)
