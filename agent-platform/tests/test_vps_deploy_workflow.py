...
class VpsDeployWorkflowTests(unittest.TestCase):
    ...
    def test_earnings_research_has_single_public_cli(self):
        ...
        self.assertIn('earnings-otm-daily.timer', workflow)
        self.assertIn('OnCalendar=*-*-* 06:00:00', workflow)
        self.assertIn('Timezone=Asia/Jerusalem', workflow)
        self.assertIn('DAILY_RUN_SCHEDULE_TZ=Asia/Jerusalem', workflow)
        self.assertIn('DAILY_RUN_SCHEDULE_HOUR=6', workflow)
        self.assertIn('DAILY_RUN_SCHEDULE_MINUTE=0', workflow)
        self.assertNotIn('\n          0 6 * * * /agents/research/bin/earnings-otm-daily.sh', workflow)
        ...
        self.assertIn('*/5 * * * * /agents/research/bin/earnings-llm-postrun-review', workflow)
        ...
        self.assertIn(r'/^0 6 \* \* \* \/agents\/research\/bin\/earnings-otm-daily\.sh$/ {next}', workflow)
