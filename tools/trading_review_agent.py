...
def ensure_review_workspace(config: dict[str, Any], pr_number: int, token: str, context: dict[str, Any]) -> dict[str, Any]:
    ...
    (artifacts / "context.json").write_text(json.dumps({k: v for k, v in context.items() if k != "diff"}, indent=2, sort_keys=True), encoding="utf-8")
    (artifacts / "diff.patch").write_text(context["diff"], encoding="utf-8")
